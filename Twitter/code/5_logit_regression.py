...
def classify_data(X, Y, num_topics, data_name=''):
    no_users = 6902 #7211
    no_topics = num_topics

    if data_name == 'model_user_event':
        clf = LbfgsL1Logistic(tol=1e-8, no_topics=num_topics, smooth_start_index=no_users+1, ignore2w=1)
        parameters = {'C': [0.01, 0.1, 1], 'D': [0.1, 1.0, 10]}
    elif data_name.startswith('model_user_fb'):
        clf = LbfgsL1Logistic(tol=1e-8, no_topics=num_topics, smooth_start_index=no_users+1, ignore2w=0)
        parameters = {'C': [1.0], 'D':[0.0]}
    elif data_name.startswith('model_user_event_fb'):
        clf = LbfgsL1Logistic(tol=1e-8, no_topics=num_topics, smooth_start_index=2*no_users+1, ignore2w=2)
        parameters = {'C': [0.01, 0.1, 1.0], 'D': [0.1, 1.0, 10.0]}
    else:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print('please, specify the number of topics used for LDA topic modeling: 50, 100 or 200, for example'
              ' \n python 5_logit_regression.py ---num_lda_topics 100')
        sys.exit(1)  # abort because of error

    num_topics = args.num_lda_topics

    data_name_suffix = ['model_c0', 'model_user', 'model_probTopic', 'model_user_probTopic', 'model_user_event',
                        'model_user_event_fb_pudt', 'model_user_event_fb_favdt']
    #data_name_suffix = ['model_c0', 'model_user', 'model_probTopic', 'model_user_probTopic', 'model_user_fb', 'model_user_event',
    #                    'model_user_event_fb_pu', 'model_user_event_fb_put', 'model_user_event_fb_pudt',
    #                    'model_user_event_fb_favdt', 'model_user_event_fb_finaldt']
    #data_name_suffix = ['model_user_event_fb_pudt']
    ...
    for i, suffix_name in enumerate(data_name_suffix):
        print(suffix_name)
        Xfile_name = 'featuresX_' + suffix_name +'_LDA'+str(num_topics)+'.npz'
        X = load_npz(Xfile_name)

        t0 = time.time()
        res = classify_data(X, Y, num_topics, suffix_name)
        model_Res[i, :] = res
        t1 = time.time()
    ...
    res_df.to_csv('feature_importance_LDA'+str(num_topics)+'.csv', sep=',')
