...
def classify_data(X_user, Y, num_topics, prefix):
    no_users = 7211
    no_topics = 100
    ...
    data_df = pd.read_csv('../data/regression_data/logit_reg_data_'+str(num_topics)+'.csv')
    ...
    print(len(neg_fb_idx), len(other_idx), len(pos_fb_idx))
    ...
    for k in range(no_bootstraps):
        ...
        rel_coefs = copy.copy(coefs[:no_users])  # coefficient for per-uer feedback estimate5:13
    ...
    np.savetxt('diff_in_accuracies_2G_LDA'+str(num_topics)+'_'+prefix+'.txt', diff_in_accuracies)
    ...
    np.savetxt('diff_in_accuracies_2groups_LDA'+str(num_topics)+'_'+prefix+'.txt', data_to_save)
...
if __name__ == '__main__':
    ...
    Xfile_name = 'featuresX_' + data_name_suffix[0] +'_'+prefix_fb+'_LDA'+str(num_topics)+'.npz'
    X_user = load_npz(Xfile_name)

    t0 = time.time()
    classify_data(X_user, Y, num_topics, prefix=prefix_fb)
    t1 = time.time()
