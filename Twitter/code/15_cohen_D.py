...
class LbfgsL1Logistic(BaseEstimator, ClassifierMixin):
    ...
    def predict_proba(self, X):
        prob = expit(safe_sparse_dot(X, self.coef_))
        return prob
...
def classify_data(X_user99, X_user50, X_user, Y, num_topics, prefix):
    no_users = 6902  # 7211
    no_topics = num_topics

    clf_uef = LbfgsL1Logistic(C=0.1, D=10, tol=1e-8, no_topics=no_topics, smooth_start_index=2 * no_users + 1,
                              ignore2w=2)

    data_coef = load_npz('bootstraps_coefs_LDA' + str(no_topics) + '_' + prefix + '.npz')
    ...
    data_fb = np.loadtxt('bootstraps_alpha_perUser_LDA' + str(num_topics) + '_' + prefix + '.txt')
    ...
    clf_uef.fit(X_user, Y)

    top1_bs = np.zeros((no_bootstraps, len(Y)))
    top50_bs = np.zeros((no_bootstraps, len(Y)))

    for k in range(no_bootstraps):
        if k % 25 == 0:
            print('bootstrap, ', k)
        clf_uef.coef_ = data_coef[k]

        pred_top1 = clf_uef.predict_proba(X_user99)
        pred_top50 = clf_uef.predict_proba(X_user50)

        top1_bs[k] = pred_top1
        top50_bs[k] = pred_top50


    top1 = np.zeros(len(pos_fb_idx))
    top50 = np.zeros(len(pos_fb_idx))
    k = -1
    for u in pos_fb_idx:
        k+=1
        sel_idxs = np.where(users_of_posts == u)[0]
        top1_val = top1_bs[:, sel_idxs]
        top50_val = top50_bs[:, sel_idxs]
        top1[k] = np.mean(top1_val)
        top50[k] = np.mean(top50_val)

        #median_diff_1_50[1][u] = (np.percentile(diff_per_user, 99.5) - np.percentile(diff_per_user, 0.5)) / 2.0

    #np.savetxt('diff_in_prob_1_50_LDA' + str(no_topics) + '_' + prefix + '.txt', median_diff_1_50)
    compute_cohen_d(top1, top50)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print(
            'please, specify the number of topics used for LDA topic modeling: 50, 100 or 200, and prefix of feedback (pu, fav, final)'
            ' for example'
    ...
    data_name_suffix = ['model_user_event_fb_99', 'model_user_event_fb_50', 'model_user_event_fb']
    ...
    Xfile_name = 'featuresX_' + data_name_suffix[0] + '_LDA' + str(num_topics) + '.npz'
    X_user99 = load_npz(Xfile_name)

    Xfile_name = 'featuresX_' + data_name_suffix[1] + '_LDA' + str(num_topics) + '.npz'
    X_user50 = load_npz(Xfile_name)

    Xfile_name = 'featuresX_' + data_name_suffix[2] + '_' + prefix_fb + '_LDA' + str(num_topics) + '.npz'
    X_user = load_npz(Xfile_name)

    t0 = time.time()
    classify_data(X_user99, X_user50, X_user, Y, num_topics, prefix=prefix_fb)
    t1 = time.time()
